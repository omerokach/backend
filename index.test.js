...
const fs = require("fs");
beforeAll(() => {
  fs.appendFileSync("./database/test1.json", 'Hello content!');
});
afterAll(() => {
...
describe("PUT route", () => {
  const taskToPost = {
    ...
    const files = ["0ca801d3-cb6d-4850-800b-106644034807.json", "test.json", "test1.json"];
  ...
  describe("DELETE route", () =>{
      it("Should delete bin by given id", async () => {
          const response = await request(app).delete("/b/test1.json");
          const files = fs.readdirSync("./database");
          expect(response.status).toBe(200);
          expect(files).not.toMatch("test1.json");
